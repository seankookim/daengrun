...
- **Number a migration against `origin/redesign-v4`, never against your fork point.** Run
  `git fetch && git ls-tree --name-only origin/redesign-v4 supabase/migrations/ | tail` at the
  moment you create the file, and the same for `supabase/tests/`. Parallel sessions branch from
  whatever the tip was when their worktree was made and then all pick "the next free number" —
  on 2026-08-13, **0078 was claimed by three sessions in one day**, and un-collided ~40
  references across migrations, suites, edge functions, client comments and three docs. Checking
  costs one command; renaming afterwards costs an afternoon. If your worktree is more than a few
  hours old, rebase before you write SQL at all.

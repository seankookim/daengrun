    ...
    <View style={{ flex: 1 }}>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 22, paddingTop: 56, paddingBottom: 30 }}>
        <Row style={{ justifyContent: 'space-between', marginBottom: 4 }}>
          ...
          <Text style={text.h2}>마이 카드</Text>
          <View style={{ width: 24 }} />
        </Row>
        <Text style={[text.dim, { textAlign: 'center', marginBottom: 16 }]}>
        ...
        <Text style={[text.h2, { marginTop: 26, marginBottom: 4 }]}>다음 도전</Text>
        <Text style={[text.dim, { marginBottom: 12 }]}>조건을 달성하면 카드가 열려요</Text>
        <View style={{ gap: 14, alignItems: 'center' }}>
          {locked.map((c) => <RunCard key={c.id} card={c} width={340} />)}
        </View>

        <View style={{ marginTop: 22, backgroundColor: '#faf8f0', borderRadius: 16, padding: 14, borderWidth: 1, borderColor: colors.line }}>

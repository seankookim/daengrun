    ...
    <View style={{ flex: 1, backgroundColor: colors.cream }}>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingTop: 56 }}>
        ...
        <View style={s.banner}>
          <View style={{ flex: 1 }}>
            <Row style={{ gap: 6 }}>
              <Text style={{ fontSize: 12.5, color: colors.volt }}>♛</Text>
              <Text style={{ fontSize: 12.5, fontWeight: '800', letterSpacing: 2, color: colors.volt }}>MEMBER</Text>
            </Row>
            <Text style={{ fontSize: 20.5, fontWeight: '900', color: '#fff', marginTop: 6 }}>
              멤버는 전 상품 <Text style={{ color: colors.volt }}>10%</Text> 할인
            </Text>
            <Text style={{ fontSize: 14, color: '#b8c4ae', marginTop: 6 }}>스토어 오픈 준비 중 — 하이 포인트와 연동돼요</Text>
          </View>
          <View style={s.bannerGo}><Text style={{ fontSize: 19.5, fontWeight: '900', color: FOREST }}>→</Text></View>
        </View>
          ...
          {products.map((p) => (
            <Pressable key={p.id} style={[s.prod, { backgroundColor: '#fff', borderWidth: 1, borderColor: '#DCD6C4' }]} onPress={() => Alert.alert(p.name, '스토어 오픈 준비 중이에요')}>
              ...
              <Row style={{ justifyContent: 'space-between', marginTop: 'auto' }}>
                <Text style={{ fontSize: 18.5, fontWeight: '900', color: FOREST }}>{p.price.toLocaleString()}원</Text>

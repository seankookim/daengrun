...
export default function Request() {
  ...
  // Memoised on the two coordinates so its identity changes exactly when its value does — the memos
  // below depend on `pickup` itself (they used to list pickup?.lat/?.lng beside a per-render object).
  const pickLat = pickupAddr?.lat != null && pickupAddr?.lng != null ? pickupAddr.lat : null;
  const pickLng = pickupAddr?.lat != null && pickupAddr?.lng != null ? pickupAddr.lng : null;

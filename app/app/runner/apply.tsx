    ...
    <ScrollView style={{ flex: 1, backgroundColor: colors.cream }} contentContainerStyle={{ padding: 16, paddingTop: 56, paddingBottom: 40 }}>
      ...
      <View style={s.tierCard}>
        <Row style={{ justifyContent: 'space-between' }}>
          <View>
            <Text style={{ fontSize: 11, color: '#b8c4ae', letterSpacing: 1.5 }}>현재 등급</Text>
            <Text style={{ fontSize: 24, fontWeight: '900', color: colors.volt, marginTop: 3 }}>{applyStatus.tier}</Text>
          </View>
          <View style={{ alignItems: 'flex-end' }}>
            <Text style={{ fontSize: 11, color: '#b8c4ae' }}>다음 등급: {applyStatus.nextTier}</Text>
            <Text style={{ fontSize: 13, fontWeight: '800', color: '#fff', marginTop: 3 }}>러닝 {applyStatus.runsToNext}회 남음</Text>
          </View>
        </Row>
          ...
          <View key={st.key}>
            ...
            >
              <View style={[s.stepDot, st.done && { backgroundColor: '#6aa53c' }, st.active && s.stepActive]}>
                {st.done ? <Text style={{ fontSize: 10, fontWeight: '900', color: '#fff' }}>✓</Text>
                  : <Text style={{ fontSize: 11, fontWeight: '900', color: st.active ? '#a97c12' : '#b3b3ab' }}>{i + 1}</Text>}
              </View>
              <View style={{ flex: 1 }}>
                <Row style={{ gap: 6 }}>
                  <Text style={{ fontSize: 14.5, fontWeight: '800', color: st.done || st.active ? FOREST : '#9a9a90' }}>{st.label}</Text>
                  {st.active && <View style={s.nowPill}><Text style={{ fontSize: 8.5, fontWeight: '900', color: '#a97c12' }}>진행 중</Text></View>}
                </Row>
                <Text style={{ fontSize: 11.5, color: colors.dim, marginTop: 2 }}>{st.desc}</Text>
              </View>
              {st.active && <Text style={{ fontSize: 13, color: '#5a7a3c', alignSelf: 'center' }}>›</Text>}
            </Pressable>
          </View>
        ))}
      </View>
      ...
      <Pressable style={s.continueBtn} onPress={() => Alert.alert('안전 교육', '모듈 5/6: 여름철 열사병 신호 (목업)')}>
        <Text style={{ fontSize: 14.5, fontWeight: '900', color: FOREST }}>교육 이어하기 — 모듈 5/6</Text>
      </Pressable>
      ...
      {TIERS.map((t) => (
        <View key={t.name} style={[s.card, { marginBottom: 8 }, t.name === applyStatus.tier && { borderColor: '#a9c47e', borderWidth: 1.6 }]}>
          <Row style={{ justifyContent: 'space-between' }}>
            <Text style={{ fontSize: 14.5, fontWeight: '900', color: FOREST }}>{t.name}</Text>
            {t.name === applyStatus.tier && (
              <View style={s.nowPill}><Text style={{ fontSize: 8.5, fontWeight: '900', color: '#4a6d1f' }}>현재</Text></View>
            )}
          </Row>
          <Text style={{ fontSize: 11.5, color: colors.dim, marginTop: 4 }}>조건: {t.req}</Text>
          <Text style={{ fontSize: 11.5, fontWeight: '700', color: '#5a7a3c', marginTop: 2 }}>혜택: {t.perk}</Text>
        </View>
      ))}

      <Text style={{ fontSize: 10.5, color: colors.dim, textAlign: 'center', marginTop: 10, lineHeight: 15 }}>

...
export function DrawButton({ title, sub, meta, ground = 'paper', art, small, coda, dot, sheen, onPress, accessibilityLabel }: Props) {
  ...
  // ⚠ 56은 minHeight(바닥)다 — 서브가 있는 코다의 실높이는 내용이 정한다(약 68pt:
  // 22+2+19 본문 + 22 패딩 + 3 립). 랩 목업의 CSS(min-height:56 + 같은 내용)도 정확히
  // 같은 ~68px로 렌더된다 — 목업과 같은 물건이고, 56을 목표로 패딩을 깎으면 랩과 달라진다.

...
//
// ⚠ THIS FILE MUST NEVER BE IMPORTED AT MODULE SCOPE FROM A ROUTE. Import `./toss-sheet`
// instead — it is a guarded lazy wrapper, and it exists because of what this comment used to say.
//
// It used to read: "imported statically so tsc checks it, but never mounted today." Both clauses
// were true and the conclusion was false. Expo Router registers every route module eagerly, so a
// static import here is EVALUATED at app launch whether or not anything mounts. `d1e2b9f` added
// the SDK to package.json without a `pod install`, so neither it nor its react-native-webview
// peer reached Podfile.lock — and every binary built from this tree crashed on the HOME screen
// with `RNCWebViewModule` missing, having never touched a payment.
//
// `TOSS_ENABLED=false` could not have helped: a flag gates behaviour, an import is evaluated at
// registration. Neither could the `if (!visible …) return null` below, for the same reason.

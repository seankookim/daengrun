...
export function copyFor(
  ...
  if (late.custody === 'post') {
    ...
    // 스윕 두 팔), 이미 나가 있는 러닝은 프로토콜에 **새로 들어오지는** 않는다.
    // 그래서 문장이 바뀐다: 지켜보는 주체를 암시하지 않고, 행동을 읽는 사람에게 준다.
    // 사실이 틀린 게 아니라(러닝은 정말 늦었다) '처리되고 있다'는 함의가 거짓이었다.
    //
    // ⚠ [정정 F2 · 2026-08-24] 위 문장의 원래 판은 「이미 나가 있는 러닝은 프로토콜에 **아예**
    // 들어오지 않는다」였다. 절반만 참이다. **진입**은 인계 전 전용이지만 **해소**는 아니다:
    // 0117:891 의 마감 팔은 체크인 행만 보고 고르고(커스터디 술어가 없다), 인계선에서 열린 체크인을
    // 닫는 코드가 0117·confirm_handoff·start_run 어디에도 없다. 그래서 10:33 에 걸린 체크인이
    // 10:52 에 시작된 러닝을 11:03 에 incident_review 로 뒤집는다 — 시작 11분 된 러닝이다.
    // 결론: 인계 후 화면은 '아무 일도 일어나지 않는다'고 **약속하면 안 된다.** 아래 두 스트립에서
  ...
  // ── 천장을 넘겼다. (resumable 은 여기서 소비된다 — 아무도 읽지 않는 필드는 아무도 검사하지
  //  않는 계약이다, codex 2026-08-21. 인계 후는 위에서 이미 처리됐으므로 여기 도달하면 인계 전이다.)
  //
  // ⚠ [정정 F3a · 2026-08-24] 첫 판은 네 문장 중 셋이 **불가능**을 단언했다:
  //   「이 예약은 진행되지 않았어요」·「이 예약은 진행할 수 없어요」·「지금은 진행할 수 없어요」
  //   ·「너무 오래 지나 시작할 수 없어요」.
  // 그걸 막는 것이 이 트리에 하나도 없었다. 서버는 천장을 모른다 — transition-booking:243 은
  // **미래** 예약만 거부하고, confirm_handoff 와 start_run 은 시계를 아예 보지 않으며,
  // 0017 의 스윕은 matching·runner_pending 만 청소한다. 즉 3시간이 지난 예약도 여전히 진행된다.
  // (실제로 Sean 의 8/4 행이 러너 홈에서 「진행할 수 없어요」 아래 82줄에 코랄 「픽업 이동 시작」을
  //  달고 있었다.) 화면이 못 한다고 말한 일을 시스템이 해내면, 그건 오늘 세 번 고친 그 거짓말이다.
  //
  // 이제 이 갈래는 **경과 사실 + 권고**만 말한다. 러너 쪽 진행 문은 runner/meetup 의 pastCeiling
  // 게이트가 실제로 닫으므로(F3b) 권고와 화면이 일치하고, 보호자 쪽은 닫을 문이 없으니 권고만 준다.
  // 천장 숫자를 문장에 박지 않는 이유: LATENESS_CEILING_MS 는 주입 가능한 제품 숫자다. 「3시간」이라
  // 적으면 그 숫자가 바뀌는 날 문장이 조용히 거짓이 된다. kick 이 이미 실제 경과를 말한다.
  if (!late.resumable) {
    return side === 'owner'

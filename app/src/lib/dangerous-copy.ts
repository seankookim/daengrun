  ...
  if (side === 'runner') {
    return {
      ...
      body: token === 'dog_dangerous_custody_refused'
        ? '동물보호법상 맹견은 러너에게 맡길 수 없어요. 이 요청은 목록에서 사라집니다.'
        : '반려견의 맹견 여부 확인이 끝나지 않았어요. 보호자가 프로필에서 답하면 다시 요청이 올 수 있어요.',
      action: null,
    };
  }
  switch (token) {
    ...
    case 'dog_dangerous_undeclared':
      return {
        token,
        title: '맹견 여부를 알려주세요',
        body: '동물보호법상 맹견은 러너에게 맡길 수 없어요. 반려견 프로필에서 한 번만 답해주시면 바로 예약할 수 있어요.',
    ...
    case 'dog_dangerous_breed_conflict':
      return {
        ...
        body: '등록된 견종은 맹견으로 분류되는데 맹견이 아니라고 신고돼 있어요. 반려견 프로필에서 견종이나 맹견 여부를 확인해주세요.',

-- ═══ 187: 팩 지도 채널 (0156) — 0156-S1~S5 · R1~R3 · P1~P3 · E1~E3 · M1~M3 · W1~W3 · N1 ═══════
--
...
--
-- ⚠ **PIN LABELS ARE SLICE-PREFIXED** (`0156-…`, CLAUDE.md). `S`/`R`/`P`/`E`/`M`/`W`/`N` collide
...
--     · `0156-R1` (public read admits everyone incl. anon) is blind to a channel that is ALWAYS
--       open. `0156-R2` (read is false while the window is closed) is blind to a channel that is
...
--     · `0156-P1` (a checked-in participant MAY publish) is blind to a write side that admits
--       everybody. `0156-P2` (rsvp-not-checked-in / stranger / anon may NOT) is blind to a write
--       side that admits nobody. Same argument, on the other door.
--     · `0156-E2` is the ONLY pin in the repo that can see the `pack channel read` POLICY at all.
...
-- | M1b | same, both VERIFY arms removed | 1126/2 = `0156-W1` + **`0156-E2`** |
...
-- | M2c | genuinely REMOVE the write policy block, VERIFY arms removed | 1125/3 = `143 W1` + `0156-W1` + **`0156-E1`** (the guest's INSERT is refused at the boundary — the door, not the name) |
-- | M3  | window ② deleted (「somebody checked in」) | 1125/3 = `0156-S1`·`S5`·`R3` |
-- | M4  | window ④ deleted (the clock band) | 1127/1 = **`0156-S4` alone** |
-- | M5  | window ③ deleted (러닝 종료) | 1127/1 = **`0156-S3` alone** |
-- | M6  | window ① deleted (session status) | 1124/4 = `0156-S2`·`R2`·`E2`·`M2` |
-- | M7  | 🔴 WRITE weakened from `checked_in` to mere membership | 1127/1 = **`0156-P2` alone** |
-- | M8  | public READ made unconditional (`return true`) | 1123/5 = `0156-S1`·`S3`·`R2`·`R3`·`E2` — and **NOT `R1`** |
...
-- | M10 | 「simplify」 §B's guard split back to 0108's single line | 1124/4 = `0156-S5`·`R1`·`W3`·`E2` |
-- | M11 | widen the public roster with `avatarUrl` | 1127/1 = **`0156-M3` alone** |
-- | M12 | disclose names while the window is CLOSED | 1127/1 = **`0156-M2` alone** |
-- | M13 | WRITE returns false for everybody | 1124/4 = `0156-S5`·**`P1`**·`P3`·`E1` — and **NOT `P2`** |
-- | M14a| `club_pack_map_roster` → SECURITY INVOKER | **APPLY ABORTS** at §E VERIFY |
-- | M14b| same, §E's definer arm removed | 1126/2 = **`0156-W2`** + `M1` |
--
...
--   independent things stand in front of it (§E VERIFY aborts the apply · `0156-W2` · the
...
begin
  ...
  -- ═══════════════════════════════════════════════════════════════════════════════════════
  -- [0156-S1] THE ANTI-INACTION PROPERTY: a session that EXISTS is not a session that is
  ...
  if v_bad = '' then call _pass('pkmap','0156-S1 창은 체크인으로 열린다 — 세션 생성만으로는 닫혀 있고(익명·호스트 둘 다 거절), 실제 체크인 뒤에 열리며, 체크인이 없는 옆 세션은 계속 닫혀 있다');
  else v_msg := v_bad; call _fail('pkmap','0156-S1 체크인 창', v_msg); end if;

  -- ═══════════ [0156-S2] 세션 종료 / 취소가 닫는다 (호스트의 확정적 종료 신호) ═══════════
  v_bad := '';
  ...
  if v_bad = '' then call _pass('pkmap','0156-S2 세션 종료(done)와 취소가 창을 닫고, full 은 닫지 않는다');
  else v_msg := v_bad; call _fail('pkmap','0156-S2 세션 상태', v_msg); end if;

  -- ═══════════ [0156-S3] 호스트의 러닝 종료가 닫는다 ═══════════
  ...
  --   0144's own suite (176) owns that function's behaviour; what 0156 owns is the CONSEQUENCE.
  v_bad := '';
  ...
  if v_bad = '' then call _pass('pkmap','0156-S3 러닝 종료(club 부킹의 run_ended_at)가 읽기·쓰기 양쪽을 닫는다 — 그리고 스탬프를 되돌리면 창이 돌아온다(이 핀이 재는 것이 그 스탬프임을 보이는 역방향 통제)');
  else v_msg := v_bad; call _fail('pkmap','0156-S3 러닝 종료', v_msg); end if;

  -- ═══════════ [0156-S4] 시계 상한 — 오직 무행동에 대한 방어 ═══════════
  ...
  if v_bad = '' then call _pass('pkmap','0156-S4 시계 상한 — 예정 +6h 를 넘기면 아무도 종료를 누르지 않아도 닫히고, -2h 이전에는 아직 열리지 않는다(체크인 창과 같은 밴드)');
  else v_msg := v_bad; call _fail('pkmap','0156-S4 시계 상한', v_msg); end if;

  -- ═══════════════════════════════════════════════════════════════════════════════════════
  -- [0156-S5] 🔴 AN ALL-동반 (Mode A) PACK IS LIVE. This is the requirement that rules out the
  ...
  --
  -- 0156's window is keyed on CHECK-IN, which every participant kind performs, so it does not
  ...
  if v_bad = '' then call _pass('pkmap','0156-S5 위탁견이 하나도 없는 동반(Mode A) 전용 팩도 살아 있다 — 부킹이 0개인 세션에서 창이 열리고, 공개 읽기와 동반 보호자의 발행이 통과하며, 로스터가 두 사람을 명명한다. 「active 부킹이 있는가」를 창으로 삼았다면 이 세션은 산책 중에 빈 지도였을 것이다');
  else v_msg := v_bad; call _fail('pkmap','0156-S5 동반 전용 팩', v_msg); end if;

  -- ═══════════════════════════════════════════════════════════════════════════════════════
  -- [0156-R1] READ IS PUBLIC. Sean 2026-08-28: 「everyone should see everyone else on the map
  ...
  if v_bad = '' then call _pass('pkmap','0156-R1 읽기는 공개다 — 익명·무관한 사용자·미체크인 참가자·게스트·호스트 전부 입장(Sean 2026-08-28 「total public」)');
  else v_msg := v_bad; call _fail('pkmap','0156-R1 공개 읽기', v_msg); end if;

  -- ═══════════ [0156-R2] 창이 닫히면 읽기도 닫힌다 — R1 의 짝 통제 ═══════════
  ...
  update club_sessions set status = 'open' where id = v_ses;      -- restore
  if v_bad = '' then call _pass('pkmap','0156-R2 창이 닫히면 읽기도 닫힌다 — 호스트조차 못 들어간다(R1 과 짝, 상수 술어로는 둘 다 만족할 수 없다)');
  else v_msg := v_bad; call _fail('pkmap','0156-R2 닫힌 창의 읽기', v_msg); end if;

  -- ═══════════ [0156-R3] 형식·치환·존재하지 않는 세션은 닫힌다 ═══════════
  v_bad := '';
  ...
  if v_bad = '' then call _pass('pkmap','0156-R3 잘못된 형식·꼬리·없는 세션·옆 세션·부킹 id 치환 전부 거절(공개여도 아무 uuid 나 열리지는 않는다)');
  else v_msg := v_bad; call _fail('pkmap','0156-R3 읽기 페일클로즈', v_msg); end if;

  -- ═══════════════════════════════════════════════════════════════════════════════════════
  -- [0156-P1] 쓰기는 체크인한 참가자 — 긍정 통제. 다섯 종류의 참가자가 전부 발행할 수 있어야
  ...
  if channel_allowed(v_topic, v_comp,   'write') is not true then v_bad := v_bad || ' 동반 보호자가 발행 못 한다(0156 전에는 이 사람도 지도에 없었다)'; end if;
  if channel_allowed(v_topic, v_guest,  'write') is not true then v_bad := v_bad || ' 개 없는 게스트가 발행 못 한다(Sean: guest is a member)'; end if;
  if v_bad = '' then call _pass('pkmap','0156-P1 체크인한 참가자 다섯 종류(호스트·담당 러너·위탁 보호자·동반 보호자·개 없는 게스트)가 전부 발행할 수 있다 — 「everyone … on the map」 은 러너만이 아니다');
  else v_msg := v_bad; call _fail('pkmap','0156-P1 발행 긍정 통제', v_msg); end if;

  -- ═══════════ [0156-P2] 쓰기는 공개가 아니다 — P1 의 짝 통제 ═══════════
  ...
  if v_bad = '' then call _pass('pkmap','0156-P2 참가만 한 사람·무관한 사용자·익명은 발행하지 못한다 — 자율가입(0134:53-61)이 있으므로 「멤버」가 아니라 「체크인」이 술어여야 한다(P1 과 짝)');
  else v_msg := v_bad; call _fail('pkmap','0156-P2 발행 부정 통제', v_msg); end if;

  -- ═══════════ [0156-P3] 토픽 치환 — 이 세션의 참가자가 옆 세션에 발행하지 못한다 ═══════════
  v_bad := '';
  ...
  if v_bad = '' then call _pass('pkmap','0156-P3 발행 권한은 토픽의 세션에 매인다 — 옆 세션의 체크인 호스트는 되고, 이 세션의 러너·게스트는 안 된다(창이 열린 상태에서 잰다)');
  else v_msg := v_bad; call _fail('pkmap','0156-P3 토픽 치환', v_msg); end if;
  ...
  -- ---------- [0156-E1] authenticated at the boundary ----------
  v_bad := '';
  ...
  if v_bad = '' then call _pass('pkmap','0156-E1 경계 실측(authenticated) — 체크인 참가자와 무관한 사용자 둘 다 SELECT 1행(공개 읽기), 발행은 체크인 참가자만: 무관한 사용자의 INSERT 는 거절되고 참가자의 INSERT 는 통과한다');
  else v_msg := v_bad; call _fail('pkmap','0156-E1 경계 authenticated', v_msg); end if;

  -- ---------- [0156-E2] 🔴 anon at the boundary — the ONLY pin that can see `pack channel read`
  ...
  update club_sessions set status = 'open' where id = v_ses;      -- restore
  if v_bad = '' then call _pass('pkmap','0156-E2 경계 실측(anon) — 로그인하지 않은 클라이언트가 SELECT 1행으로 입장하고(pack channel read 를 볼 수 있는 유일한 핀), 발행은 거절되며, 세션이 종료되면 익명도 0행이 된다');
  else v_msg := v_bad; call _fail('pkmap','0156-E2 경계 anon', v_msg); end if;

  -- ---------- [0156-E3] presence 프로브는 닫혀 있고, 옛 네 가족은 익명에게 그대로 닫혀 있다 ----
  ...
  if v_bad = '' then call _pass('pkmap','0156-E3 presence 프로브는 닫혀 있고, uid 가드를 아래로 옮긴 뒤에도 run2·bk·club-chat·chat 네 가족은 익명에게 그대로 닫혀 있다(가드가 삭제된 게 아니라 이동했음을 재는 회귀 핀)');
  else v_msg := v_bad; call _fail('pkmap','0156-E3 가족 격리', v_msg); end if;

  -- ═══════════════════════════════════════════════════════════════════════════════════════
  -- [0156-M1] 로스터 — 익명이 부를 수 있고, 값이 맞다. ⚠ VALUES, not a key set: a body returning
  ...
  if v_bad = '' then call _pass('pkmap','0156-M1 로스터는 익명이 부를 수 있고 값이 맞다 — 체크인한 5명만(참가만 한 사람은 이름조차 없다), 담당 러너 isRunner=true·게스트 false·호스트 isHost=true, 이름은 서버의 profiles 값(브로드캐스트 페이로드가 아니라 이것이 진실이다)');
  else v_msg := v_bad; call _fail('pkmap','0156-M1 로스터 값', v_msg); end if;

  -- ═══════════ [0156-M2] 창이 닫히면 이름을 내주지 않는다 + 없는 세션은 not_found ═══════════
  v_bad := '';
  update club_sessions set status = 'done' where id = v_ses;
  v_j := club_pack_map_roster(v_ses);
  ...
  if v_bad = '' then call _pass('pkmap','0156-M2 창이 닫히면 people=[] 이고 windowOpen=false 만 답한다(이름은 산책 중에만 공개된다) · 되돌리면 돌아온다 · 없는 세션은 not_found');
  else v_msg := v_bad; call _fail('pkmap','0156-M2 닫힌 로스터', v_msg); end if;

  -- ═══════════ [0156-M3] 공개 로스터가 내주지 않는 것 — 키 집합을 고정한다 ═══════════
  ...
  if v_bad = '' then call _pass('pkmap','0156-M3 공개 로스터의 키 집합은 고정이다 — 최상위 7키, 사람 5키, 그리고 phone·avatar·booking·좌표는 어디에도 없다(전화는 호스트 전용이라는 Sean 의 2차 답변이 여기서 실행 가능해진다)');
  else v_msg := v_bad; call _fail('pkmap','0156-M3 로스터 키 집합', v_msg); end if;

  -- ═══════════════════════════════════════════════════════════════════════════════════════
  -- [0156-W1] 배선 — 정책이 있고, 모양이 맞고, 0108 의 셋은 그대로다
  -- ═══════════════════════════════════════════════════════════════════════════════════════
  v_bad := '';
  ...
  if v_bad = '' then call _pass('pkmap','0156-W1 배선 — pack channel read(SELECT/anon+authenticated) + pack channel write(INSERT/authenticated 전용), 둘 다 pack-%% 울타리 안, 0108 의 셋은 그대로, 총 5개');
  else v_msg := v_bad; call _fail('pkmap','0156-W1 배선', v_msg); end if;

  -- ═══════════ [0156-W2] ACL 과 함수 형상 — VERIFY 블록이 apply 때 재는 것을 상시 핀으로도 ═══════
  -- ⚠ A property checked at apply and never pinned is protected exactly until someone recreates
  --   the function (CLAUDE.md). 0156 §E and this pin are the same assertions in two places on
  ...
  if v_bad = '' then call _pass('pkmap','0156-W2 ACL — 네 함수 전부 definer+본문 search_path, 임의 uid 프로브는 양 롤에 닫혀 있고 _club_pack_window 는 서버 전용, my_channel_allowed 와 club_pack_map_roster 는 anon+authenticated 에 열려 있으며 0103 오라클은 계속 닫혀 있다');
  else v_msg := v_bad; call _fail('pkmap','0156-W2 ACL', v_msg); end if;

  -- ═══════════ [0156-W3] anon 확장의 경계 — 열어준 것이 정확히 무엇인지 재는 핀 ═══════════
  -- 0156 grants `my_channel_allowed` to anon, which 0108 deliberately did not. This pin owns the
  ...
  reset role;
  if v_bad = '' then call _pass('pkmap','0156-W3 익명에게 연 것은 pack 읽기 하나뿐 — 같은 래퍼로 pack write·run2·bk·chat·club-chat 은 전부 false(uid 가 NULL 이라 신원 가드가 거절한다)');
  else v_msg := v_bad; call _fail('pkmap','0156-W3 anon 확장 경계', v_msg); end if;

  -- ═══════════ [0156-N1] 입력 가드 — 분리된 두 가드가 둘 다 살아 있다 ═══════════
  ...
  if v_bad = '' then call _pass('pkmap','0156-N1 입력 가드 — null 토픽·null op·알 수 없는 op(익명 포함)·하이픈 없는 pack·은퇴한 run- 전부 거절. pack 읽기 팔이 op 검사 위로 올라가지 않았음을 재는 핀');
  else v_msg := v_bad; call _fail('pkmap','0156-N1 입력 가드', v_msg); end if;

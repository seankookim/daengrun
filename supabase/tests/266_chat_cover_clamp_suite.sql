...
-- a property trunk already has but nothing asserted on purpose.
--
...
--        ways. In the COMMENT-STRIPPED body, the release statement alone (from `update
--        notifications` to `return query`) carries the thread scope `m.thread_id = p_thread`, the
--        sender exclusion, `m.created_at > v_at` AND the clamp conjunct `now() > v_at`, and sits
--        below the party gate. Scoped to the release statement because §A's acknowledged-message
--        lookup also says `m.thread_id = p_thread` (the reason 0228 VERIFY ④ could not see s2).
--        NO-SOURCE arm per function and a two-sided stripper control.
--
-- ─── WHAT THIS SUITE DOES NOT PROVE (prose — named gaps, not pins) ───
--   · §B's THREAD SCOPE IS NOT BEHAVIOURALLY OBSERVABLE after 0235. In §B `v_at` is
--     `greatest(stored, now())` ≥ now(), so the clamp conjunct `now() > v_at` is false for every
--     row and the covering subquery is empty whatever its other conjuncts say: dropping §B's
--     `m.thread_id = p_thread` changes no outcome any fixture can produce. It is asserted
--     structurally only (S1 + 0235 VERIFY). Not a blind pin — the property has no door.
--   · The commit-order race (0228 §0d ①) and its 0235 neighbour (0235 §0d ②): one session cannot
--     interleave two commits.
--
-- ─── FIXTURE NOTES ───
...
declare
  ...
  m1 bigint; m_fut bigint; m_tw bigint;
...
begin
  ...
  begin
    ...
    foreach v_fn in array array['chat_mark_read_to', 'chat_mark_read'] loop
      ...
      -- The RELEASE STATEMENT alone: §A's acknowledged-message lookup also scopes by thread, so a
      -- whole-body match would be satisfied by that lookup (the reason s2 was invisible).
      ...
      if (v_rel ~ 'not\s+exists\s*\(\s*select\s+1\s+from\s+chat_messages\s+m') is not true
      then v_bad := v_bad || ' ' || v_fn || ': the covering subquery is gone'; end if;

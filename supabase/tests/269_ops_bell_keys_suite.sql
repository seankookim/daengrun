...
-- ═══        0238-A1 · A2 · F1 · F2 · F3 · G1 · G2 · G3 · G4 · D1 · S1, tag `obk`
--
...
-- Each pin plants THREE shapes, chosen so that each of the identity's THREE conjuncts — the kind,
-- the recipient's roster row, and that row's CLASS — is separately observable (264 `0233-B4`'s
-- design, widened by the 0238 fix round):
...
--   · CROSS-CLASS look-alike (ⓢ) [fix round]: an operator seated ONLY on the OTHER class (opsP, on
--     `payout_due`, forges the three `return_strand` bells; opsR, on `return_strand`, forges the
--     `payout_due` bell) rewrites their own row exactly as the stranger does. Kind system AND a
--     recipient with a roster row — so ONLY the `orr.event_class = <class>` conjunct excludes it.
--     The first cut had no fixture where 「any roster」 and 「this class's roster」 disagree, so
--     deleting the class conjunct at all 11 sites reddened only S1's source regex (measured by the
...
--   · D1 **「IS OR WAS SEATED」 [fix round].** After the real bells rang (tick 1), EVERY seat of both
--        classes is DEACTIVATED (`active = false`, `ops_roster_set`'s only removal — including other
--        suites' committed operators, who received tick 1 too) and a second active operator is
--        seated on each class. On tick 3 no really-rung booking rings again (while a
--        fresh control of each shape DOES ring the new operator — the 0 is not an empty roster), all
--        three lists still date each really-rung booking (notified_at not NULL), and with every
--        deadline off both admissions still list the really-rung rows. A former operator's bell was
--        DELIVERED; 「active only」 would re-ring it and drop it from the desk. The first cut left this
--        choice to S1's spelling alone (executing review: `and orr.active` at all 11 sites → 1631/1,
--        S1 only).
...
--     the fix round adds the 11 class-conjunct deletions, each measured against its own site).
...
--     The same holds for D1 [fix round]: 「active only」 on ONE half of arm ⓕ's or arm ⓖ's candidate /
--     re-check pair is S1-only (measured: sites 2, 3, 4, 5, 6 alone), while each PAIR reddens D1.
--   · A profile that IS OR WAS seated on the class can still forge on their OWN row through ⓢ (kind
--     system, recipient on the roster) — the named residual 0233 §C already carries, and it includes
--     a DEACTIVATED (former) operator: deactivation does not revoke it (measured by the executing
--     review and re-measured by the fixer: a former `return_strand` operator's rewritten row silenced
...
--     operator of each class (other suites may have seated more). Tick 3 (D1) runs last, after
--     the admission world, with tick 1's operators deactivated and a second operator seated.
--  ④ All three ticks share one transaction, so every real ring carries the SAME `now()`: a re-ring
--     cannot be told from the first ring by its instant. D1 therefore reads COUNTS for the sweep
--     sites and NULL-vs-set for the lists (a list-only 「active」 narrowing drops the instant to
--     NULL; a sweep-side one adds a row to the new operator).
...
do $$
declare
  oo uuid; sx uuid; opsR uuid; opsP uuid; rr uuid; dg uuid; rt uuid; opsR2 uuid; opsP2 uuid;
  ...
  -- bookings
  aReal uuid; aOp uuid; aSt uuid; aCtl uuid; aXc uuid;
  fReal uuid; fOp uuid; fSt uuid; fCtl uuid; fXc uuid; fNbOp uuid; fNbSt uuid; fNbXc uuid;
  gsReal uuid; gsOp uuid; gsSt uuid; gsCtl uuid; gsXc uuid;
  geReal uuid; geOp uuid; geSt uuid; geCtl uuid; geXc uuid; gNbOp uuid; gNbSt uuid; gNbXc uuid;
  -- D1: tick 3's controls (fresh, never rung — they MUST ring the new operators)
  aCtl3 uuid; fCtl3 uuid; gsCtl3 uuid; geCtl3 uuid;
  -- captured world
  ...
  lSealD jsonb; lRetD jsonb; lCusD jsonb; lRetNullD jsonb; lCusNullD jsonb;
  stage_err text;
  -- scratch
...
begin
  ...
  begin
    ...
    on conflict (profile_id, event_class) do update set active = true;
    -- the cross-class forgers must sit where 「any roster」 and 「this class's roster」 DISAGREE:
    -- each on its own class, and on no row (active or not) of the other
    w := w || jsonb_build_object('xcSeat',
      (select count(*) from ops_recipients where profile_id = opsP and event_class = 'payout_due')
      + (select count(*) from ops_recipients where profile_id = opsR and event_class = 'return_strand'),
      'xcOff',
      (select count(*) from ops_recipients where profile_id = opsP and event_class = 'return_strand')
      + (select count(*) from ops_recipients where profile_id = opsR and event_class = 'payout_due'));
    ...
    aXc  := t_obk_booking(oo, rr, dg, rt, 'sealed');   fXc  := t_obk_booking(oo, rr, dg, rt, 'strand');
    gsXc := t_obk_booking(oo, rr, dg, rt, 'start');    geXc := t_obk_booking(oo, rr, dg, rt, 'endrun');
    forged := jsonb_build_object(
      -- cross-class (ⓢ): the OTHER class's operator rewrites their own row
      'aXc',  t_obk_forge_self(opsR, aXc, T_SEAL),    'fXc',  t_obk_forge_self(opsP, fXc, T_STR),
      'gsXc', t_obk_forge_self(opsP, gsXc, T_START),  'geXc', t_obk_forge_self(opsP, geXc, T_END)) || jsonb_build_object(
    ...
    w := w || jsonb_build_object('pre',
      ...
      + t_obk_n(aXc, T_SEAL, opsP) + t_obk_n(fXc, T_STR, opsR) + t_obk_n(gsXc, T_START, opsR)
      + t_obk_n(geXc, T_END, opsR));
    ...
    w := w || jsonb_build_object(
      'aXc', t_obk_n(aXc, T_SEAL, opsP),     'aXcAt', t_obk_at(aXc, T_SEAL, opsP),
      'fXc', t_obk_n(fXc, T_STR, opsR),      'fXcAt', t_obk_at(fXc, T_STR, opsR),
      'gsXc', t_obk_n(gsXc, T_START, opsR),  'gsXcAt', t_obk_at(gsXc, T_START, opsR),
      'geXc', t_obk_n(geXc, T_END, opsR),    'geXcAt', t_obk_at(geXc, T_END, opsR));
    ...
    fNbXc := t_obk_booking(oo, rr, dg, rt, 'strand');   gNbXc := t_obk_booking(oo, rr, dg, rt, 'start');
    forged := forged || jsonb_build_object(
      'fNbXc', t_obk_forge_self(opsP, fNbXc, T_STR),   'gNbXc', t_obk_forge_self(opsP, gNbXc, T_START)) || jsonb_build_object(
    ...
    -- ── D1: tick 1's operators become FORMER operators; a second operator is seated on each class ─
    update ops_flags set return_strand_minutes = 60, custody_start_strand_minutes = 30,
                         custody_end_strand_minutes = 30 where id;
    opsP2 := t_user('obk_ops_pay2', 'owner');
    opsR2 := t_user('obk_ops_ret2', 'owner');
    insert into ops_recipients (profile_id, event_class, active) values (opsR2, 'return_strand', true)
    on conflict (profile_id, event_class) do update set active = true;
    insert into ops_recipients (profile_id, event_class, active) values (opsP2, 'payout_due', true)
    on conflict (profile_id, event_class) do update set active = true;
    -- EVERY earlier seat of both classes goes former, not only opsP/opsR: other suites commit their
    -- own seated operators, tick 1 rang them too, and one of their rows still ACTIVE would keep an
    -- 「active only」 identity satisfied — the fixture would sit in the agreement zone. (Measured: the
    -- first cut deactivated only opsP/opsR and `and orr.active` at all 11 sites left D1 green.)
    update ops_recipients set active = false
     where event_class in ('payout_due', 'return_strand') and profile_id not in (opsP2, opsR2);
    w := w || jsonb_build_object('dFormer',
      (select count(*) from ops_recipients where profile_id in (opsP, opsR) and not active),
      'dStillActive',
      (select count(*) from ops_recipients
        where event_class in ('payout_due', 'return_strand') and active and profile_id not in (opsP2, opsR2)),
      'dNew',
      (select count(*) from ops_recipients
        where (profile_id, event_class) in ((opsP2, 'payout_due'), (opsR2, 'return_strand')) and active));
    aCtl3  := t_obk_booking(oo, rr, dg, rt, 'sealed');  fCtl3  := t_obk_booking(oo, rr, dg, rt, 'strand');
    gsCtl3 := t_obk_booking(oo, rr, dg, rt, 'start');   geCtl3 := t_obk_booking(oo, rr, dg, rt, 'endrun');
    -- ── tick 3 ───────────────────────────────────────────────────────────────────────────────
    perform sweep_run_end_recovery();
    w := w || jsonb_build_object(
      'aRealD', t_obk_n(aReal, T_SEAL, opsP2),   'aCtl3', t_obk_n(aCtl3, T_SEAL, opsP2),
      'fRealD', t_obk_n(fReal, T_STR, opsR2),    'fCtl3', t_obk_n(fCtl3, T_STR, opsR2),
      'gsRealD', t_obk_n(gsReal, T_START, opsR2), 'gsCtl3', t_obk_n(gsCtl3, T_START, opsR2),
      'geRealD', t_obk_n(geReal, T_END, opsR2),   'geCtl3', t_obk_n(geCtl3, T_END, opsR2));
    lSealD := t_obk_list('ops_sealed_unsettled', opsP2);
    lRetD  := t_obk_list('ops_stranded_returns', opsR2);
    lCusD  := t_obk_list('ops_stranded_custody', opsR2);
    update ops_flags set return_strand_minutes = null, custody_start_strand_minutes = null,
                         custody_end_strand_minutes = null where id;
    lRetNullD := t_obk_list('ops_stranded_returns', opsR2);
    lCusNullD := t_obk_list('ops_stranded_custody', opsR2);
  ...
  begin
    ...
    if (w->>'aXc') is distinct from '1' then v_bad := v_bad || ' 🔴 a return_strand operator''s rewritten system row silenced the payout_due bell (' || coalesce(w->>'aXc', 'NULL') || ')'; end if;
    ...
    if v_bad = '' then call _pass('obk','0238-A1 arm ⓐ 「정산 미완료 — 확인 필요」: the operator-owner''s kind=booking look-alike (party insert), a stranger''s row rewritten to system and a return_strand-only operator''s row rewritten to system (self update), each asserted landed, do not silence the bell — one system row each, like the control — and a real earlier ring still suppresses (rolled back)');
  ...
  begin
    ...
    if lSeal is null or lSeal ? 'raised' then v_bad := v_bad || ' list: ' || coalesce(lSeal::text, 'NULL');
      ...
      for r in select * from (values (aOp::text, w->>'aOpAt'), (aSt::text, w->>'aStAt'), (aXc::text, w->>'aXcAt'), (aReal::text, w->>'aRealAt')) x(id, real_at) loop
    ...
    if v_bad = '' then call _pass('obk','0238-A2 ops_sealed_unsettled: notified_at is the real bell''s instant for all three forged bookings (never the forged 2000-01-01) and for the really-rung one');
  ...
  begin
    ...
    if (w->>'fXc') is distinct from '1' then v_bad := v_bad || ' 🔴 a payout_due operator''s system row silenced the return_strand bell (' || coalesce(w->>'fXc', 'NULL') || ')'; end if;
    ...
    if v_bad = '' then call _pass('obk','0238-F1 arm ⓕ 「반환 좌초 — 확인 필요」: none of the three look-alikes (operator-owner, stranger, payout_due-only operator) silences the return_strand bell (one system row each, like the control); a real earlier ring still suppresses (rolled back)');
  ...
  begin
    ...
    if lRet is null or lRet ? 'raised' then v_bad := v_bad || ' list: ' || coalesce(lRet::text, 'NULL');
      ...
      for r in select * from (values (fOp::text, w->>'fOpAt'), (fSt::text, w->>'fStAt'), (fXc::text, w->>'fXcAt'), (fReal::text, w->>'fRealAt')) x(id, real_at) loop
    ...
    if v_bad = '' then call _pass('obk','0238-F2 ops_stranded_returns: notified_at is the real strand bell''s instant for all three forged bookings and the really-rung one');
  ...
  begin
    ...
    if lRetNull is null or lRetNull ? 'raised' then v_bad := v_bad || ' list: ' || coalesce(lRetNull::text, 'NULL');
      ...
      if (lRetNull->'rows'->fNbXc::text) is not null then v_bad := v_bad || ' 🔴 a payout_due operator''s system row put a never-rung strand on the list'; end if;
    end if;
    if v_bad = '' then call _pass('obk','0238-F3 ops_stranded_returns with the deadline off: a never-rung strand carrying any of the three look-alikes is NOT listed; the really-rung strand IS');
  ...
  foreach k in array array['gs', 'ge'] loop
    begin
      ...
      if (w->>(k || 'Xc')) is distinct from '1' then v_bad := v_bad || ' 🔴 a payout_due operator''s system row silenced the return_strand bell (' || coalesce(w->>(k || 'Xc'), 'NULL') || ')'; end if;
      ...
      if v_bad = '' then call _pass('obk', split_part(v_txt, ' ', 1) || ' arm ⓖ ' || case k when 'gs' then '「러닝 시작 좌초」' else '「러닝 종료 좌초」' end
                                     || ': none of the three look-alikes (operator-owner, stranger, payout_due-only operator) silences the return_strand bell (one system row each, like the control); a real earlier ring still suppresses (rolled back)');
  ...
  begin
    ...
    if lCus is null or lCus ? 'raised' then v_bad := v_bad || ' list: ' || coalesce(lCus::text, 'NULL');
      ...
      for r in select * from (values (gsOp::text, w->>'gsOpAt'), (gsSt::text, w->>'gsStAt'), (gsXc::text, w->>'gsXcAt'), (gsReal::text, w->>'gsRealAt'),
                                      (geOp::text, w->>'geOpAt'), (geSt::text, w->>'geStAt'), (geXc::text, w->>'geXcAt'), (geReal::text, w->>'geRealAt')) x(id, real_at) loop
    ...
    if v_bad = '' then call _pass('obk','0238-G3 ops_stranded_custody: notified_at is the real custody bell''s instant for all three forged bookings of each shape and for the really-rung ones');
  ...
  begin
    ...
    if lCusNull is null or lCusNull ? 'raised' then v_bad := v_bad || ' list: ' || coalesce(lCusNull::text, 'NULL');
      ...
      if (lCusNull->'rows'->gNbXc::text) is not null then v_bad := v_bad || ' 🔴 a payout_due operator''s system row put a never-rung custody on the list'; end if;
    end if;
    if v_bad = '' then call _pass('obk','0238-G4 ops_stranded_custody with both thresholds off: a never-rung custody carrying any of the three look-alikes is NOT listed; the really-rung one IS');
  ...
  -- ══════════════════════════════════════════════════════════════════════════════════════════
  -- [0238-D1] 「is OR WAS seated」: a former operator's real ring still counts, at every site
  -- ══════════════════════════════════════════════════════════════════════════════════════════
  -- Divergence zone for 「active only」 vs 「active or not」: a ring delivered to an operator who is
  -- no longer active. Sweep sites read as COUNTS to the new operator (fixture note ④: the three ticks
  -- share one now(), so a re-ring is invisible by instant); list sites read as NULL-vs-set; the
  -- admission sites read with every deadline off.
  begin
    v_bad := '';
    if stage_err is not null then v_bad := v_bad || ' staging raised: ' || stage_err; end if;
    if (w->>'dFormer') is distinct from '2' or (w->>'dStillActive') is distinct from '0' or (w->>'dNew') is distinct from '2'
      then v_bad := v_bad || ' FIXTURE: not in the divergence zone — every earlier seat of both classes must be former and only the two new operators active (opsP/opsR former ' || coalesce(w->>'dFormer', 'NULL') || '/2, other seats still active ' || coalesce(w->>'dStillActive', 'NULL') || '/0, new seats ' || coalesce(w->>'dNew', 'NULL') || '/2)'; end if;
    for r in select * from (values ('aReal1', 'aCtl3', 'aRealD', 'seal'), ('fReal1', 'fCtl3', 'fRealD', 'strand'),
                                   ('gsReal1', 'gsCtl3', 'gsRealD', 'custody start'), ('geReal1', 'geCtl3', 'geRealD', 'custody end')) x(k1, kc, kd, nm) loop
      if (w->>r.k1) is distinct from '1' then v_bad := v_bad || ' FIXTURE: tick 1 did not ring the real ' || r.nm || ' booking'; end if;
      if (w->>r.kc) is distinct from '1' then v_bad := v_bad || ' CONTROL: tick 3 rang the new operator ' || coalesce(w->>r.kc, 'NULL') || ' time(s) for a fresh ' || r.nm || ' booking (1) — a 0 below would prove nothing'; end if;
      if (w->>r.kd) is distinct from '0' then v_bad := v_bad || ' 🔴 the ' || r.nm || ' bell rang AGAIN (' || coalesce(w->>r.kd, 'NULL') || ') because its first ring went to an operator no longer active'; end if;
    end loop;
    for r in select * from (values ('ops_sealed_unsettled', lSealD, aReal::text), ('ops_stranded_returns', lRetD, fReal::text),
                                   ('ops_stranded_custody', lCusD, gsReal::text), ('ops_stranded_custody', lCusD, geReal::text)) x(fn, l, id) loop
      if r.l is null or r.l ? 'raised' then v_bad := v_bad || ' ' || r.fn || ' list: ' || coalesce(r.l::text, 'NULL');
      elsif (r.l->'rows'->r.id) is null then v_bad := v_bad || ' ' || r.fn || ': the really-rung ' || r.id || ' is not listed with its deadline on';
      elsif (r.l->'rows'->r.id->>'notified_at') is null then v_bad := v_bad || ' 🔴 ' || r.fn || ': notified_at went NULL for ' || r.id || ' once its operator was deactivated';
      end if;
    end loop;
    for r in select * from (values ('ops_stranded_returns', lRetNullD, fReal::text), ('ops_stranded_custody', lCusNullD, gsReal::text)) x(fn, l, id) loop
      if r.l is null or r.l ? 'raised' then v_bad := v_bad || ' ' || r.fn || ' list (deadline off): ' || coalesce(r.l::text, 'NULL');
      elsif (r.l->'rows'->r.id) is null then v_bad := v_bad || ' 🔴 ' || r.fn || ': with the deadline off the really-rung ' || r.id || ' dropped off the desk once its operator was deactivated';
      end if;
    end loop;
    if v_bad = '' then call _pass('obk','0238-D1 a ring delivered to a now-DEACTIVATED operator still counts: no second bell on tick 3 for the seal, strand, custody-start or custody-end booking (while a fresh control of each rings the newly seated operator), notified_at stays set on all three lists, and with every deadline off both admissions still list the really-rung rows (rolled back)');
    else v_msg := v_bad; call _fail('obk','0238-D1 former-operator ring', v_msg); end if;
  exception when others then call _fail('obk','0238-D1 former-operator ring', sqlerrm); end;

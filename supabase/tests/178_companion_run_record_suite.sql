-- ═══ 178 동반 러닝 기록 — 0146 pins (C1~C10 · G1~G2) ═══
--
...
-- 0146 adds `session_record_companion_run`, which upserts the ONE `participant_activities` row
-- for (session, caller).
--
...
begin
  ...
  -- The whole shape of 0146 is an UPSERT, and it is an upsert BECAUSE 0030:262 got there first.
  ...
  begin
    ...
    else call _pass('crr','C0 체크인이 이미 checkin_only 행을 만들어 둔다 — 0146은 그래서 UPSERT다');
    end if;
  ...
  -- legitimately name either dog. 0146's `custody = 'owner_handled'` conjunct is what settles it.
  begin
    perform set_config('request.jwt.claim.sub', o5::text, false);
...
begin
  ...
  --   replaced matches every grep that hunts for that statement (CLAUDE.md, 2026-08-26), and 0146

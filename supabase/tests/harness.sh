...
suite 151_flip_blockers_suite.sql   # 0116 flip-blockers: the four money/server defects that are inert while charging is off and real the day it flips. B1 the sweep anchors on runs.settled_at, not on the stop (0083 §0f's one-line handoff, collected — a charge could mint for a dog still on the leash) · B2 the club cancel fee reaches bookings.cancel_fee AND the runner's ledger, with every number compared against the club_fee_items rows the existing code already wrote (plumbing only — which of the two ladders governs is Sean's open ruling) · B3 one unparseable next_retry_at fails ITS OWN ROW: the pin builds a vault stub because without a secret dispatch_due_charges returns 0 whether or not it was poisoned, and it first proves the poison by requiring the OLD open-coded predicate to raise on the same table · B4 the due rule agrees with collect-charges' isDue() arm for arm, cap pinned against a literal · B5-B8 party gates on the four definers that took a caller-supplied id and contained no auth.uid() at all, each pinned BOTH ways plus the server (no-JWT) caller that must keep working — B1~B8

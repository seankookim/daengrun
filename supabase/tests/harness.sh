...
suite 140_booking_insert_party_guard_suite.sql   # 0105 booking INSERT party guard (bookings owner insert constrained only owner_id+status, so a client could name ANOTHER user's dog and an arbitrary victim as runner_id — measured ACCEPTED against production. The row is what is_booking_party() reads, so a forged draft unlocked chat/reviews/notifications aimed at the victim. B5/B6 are positive controls: booking creation and the service_role money path must survive — B1~B7)
suite 144_revoke_truncate_suite.sql   # 0109 revoke TRUNCATE from client roles (RLS never covered this verb; production default ACL is arwdDxtm so 65/68 tables held it and every new table would — T1 executes truncate as anon/authenticated on bookings/drops/routes, T2 enumerates, T3 proves a table born after 0109 does not regain it, T4 is the service_role/owner positive control — T1~T4)

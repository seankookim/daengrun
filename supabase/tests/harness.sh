...
suite 155_location_law_suite.sql   # 0120 위치정보법: the one-year destruction floor for runs.trace (시행령 제26조의2) + the 제16조 이용·제공 사실 확인자료 ledger. Both were entirely unbuilt — 17 crons and NO deleter for runs.trace anywhere in the repo, and a policy (privacy-policy.md:95) promising a 열람권 with no table behind it. L1/L2 are the purge's two halves and L2 is the one that matters: `where ended_at < now() - cap` fails OPEN on a nullable column, so the run that never ended is kept forever — anchor = least(greatest(ended_at, started_at, bookings.created_at), now()). L3 pins that nothing is destroyed silently (the applied cap is RECORDED per row, not inferred), L4 bounded/resumable/oldest-first with p_limit=0 as a dry run, L5 the row survives and only the column is emptied, L6 the exact column set of both new tables (a ledger holding coordinates would defeat the purge under an audit label), L7 the pin is on the cron.job ROW not the function (0060:144 — a function sat unscheduled for months while a comment claimed it ran; 00_shim gained a pg_cron stub in the same commit so this is a gate, not prose), L8 the purge never touches the ledger. L10/L10c/L11 close runs.trace to client roles the way 0113 closed routes — table revoke + a 13-column whitelist, because a bare column revoke against a table grant is a no-op — while every shipped query shape, saveRunTrace's UPDATE and service_role all survive. L13/L14 every provided read leaves a row (owner reads are 제3자 제공 and the subject is whose DEVICE it was, not who asked), L15 party gate first with 부재 == 타인, L16 zero points provided = zero rows, L17 the dedup is a WINDOW not a suppression, L18 the record is a PRECONDITION of the provision (ledger made unwritable → the coordinates do not come out), L19 the 열람권 window returns the subject's own rows only, L20 0115 §F's watchdog re-derived from delete_my_account_tx's own prosrc for the two tables it was armed for in advance, L21 both tables sealed from client roles by privilege AND by RLS — L1~L21

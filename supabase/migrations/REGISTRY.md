...
| 0111 | `0111_booking_entry_rebuild.sql` | 146 | 🔴 HIGH — supersedes the reviewer-rejected 0105 (booking-party forgery, the last open /cso P0): revoke client INSERT on `bookings` + drop `bookings owner insert`; close `recurring_series` client writes (INSERT/UPDATE/DELETE revoked; `grant update (paused)`; policies split with explicit with_check); ownership re-check in `generate_recurring_bookings` (continue + warning); revoke client I/U/D on `slot_holds`; delete 0105 + suite 140 + the HELD line in the same commit. Companion (not a migration): `create-booking-hold` stops taking `runner_id` from the body (400). Contract: `docs/contracts/booking-entry-rebuild-contract.md` (attacked in a scratch cluster, 21/21; B-3 reproduced). NOT closed by this slice: the legit nomination chain (own dog → payment_ok → request_runner) — `is_booking_party` status filter is the adjacent slice, shape decided by Sean's D1/D2. (announcer v3, tree `p0-booking-entry`, branch `claude/p0-booking-entry`) | **CLAIMED 2026-08-19 evening** — trust offline; announcer-directed under Sean's "full speed" default (queue §0-decies A) |

...
| 0096 | `0096_return_confirm_after_escalation.sql` | 132 | 🔴 ⑫ deadlock — a gated runner with no party-reachable exit (`claude/run-end-flow-continuation-d9c485`) | **CLAIMED 2026-08-14** — composition defect across three correct migrations: `0092`'s gate blocks a runner until both return stamps land · `0089` removed the party force · `0083`'s 2h sweep escalates to `incident_review`, after which `confirm_return_tx:39` raises `not_active` so **neither party can ever clear it**. Runner permanently unable to earn; nothing pages ops. Fix = let the parties stamp from `incident_review` (custody fact) WITHOUT settling (money stays `active`-only, money is another session's surface), plus ops detection. ⚠ Re-creates `confirm_return_tx` ←0083; 119's pins move in the same slice. |
| 0097 | *(next free)* | 133 | — | available |

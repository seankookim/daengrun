...
| 0092 | `0092_runner_work_gate.sql` | 128 | (`claude/run-end-flow-continuation-d9c485`) | **TAKEN BY FILE — ROW WAS MISSING.** Recorded here by another session that hit the pre-push hook trying to take 0092. Their file is on origin; no row ever was. Two-sided rule in action: reading this table alone said 0092 was free, and it was not. If this row is wrong in its details, fix it — but do not delete it and leave the slot looking empty again. |

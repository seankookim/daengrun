...
| `supabase/tests/00_shim.sql` (realtime block only) | trust (`claude/deploy-edge-functions-money-68e990`, worktree lucid-neumann-580f5e) | **shared** | 2026-08-15 | Add a `realtime` shim (schema + `messages` + `topic()`) so 0103's broadcast policies can be tested at the boundary rather than only as a predicate. Purely additive — no existing suite references `realtime`, so nothing can regress. ⚠ Global shim: tell me before editing the realtime block.

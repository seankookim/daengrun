...
| 0097 | `0097_unsettled_run_detection.sql` | 133 | the alarm `0096` removed — a runner unpaid for work already done (`claude/run-end-flow-continuation-d9c485`) | **CLAIMED 2026-08-14** — found by money reviewing `0096`: before it, an escalated booking left the runner gated AND unpaid (loud); after it, un-gated and still unpaid (quiet — a runner back at work surfaces nothing). `0096` fixed the acute half and in doing so removed the thing that made someone notice the chronic half. ⚠ money's suggested shape — a column on `ops_gated_runners` — CANNOT work: that function requires a missing stamp, so the row disappears at exactly the moment the quiet case begins. Needs its own predicate (ended run · no settlement · regardless of stamps). DETECTION ONLY — the money exit (`0083 §0h`) is money's slice and is not touched here. |
| 0098 | *(next free)* | 134 | — | available |
...
|---|---|---|---|---|---|
| `supabase/migrations/0095_club_critical_titles_rls.sql` · `supabase/tests/131_club_critical_titles_suite.sql` | trust (`claude/deploy-edge-functions-money-68e990`, worktree deploy-edge-functions-money-68e990) | exclusive | 2026-08-14 | Enable RLS + revoke anon write on `club_critical_titles`. New files only. |
| `docs/decisions/awaiting-sean.md` (§1 only) | trust (`claude/deploy-edge-functions-money-68e990`, worktree deploy-edge-functions-money-68e990) | **shared** | 2026-08-14 | Correct §1: the `profiles` P0 is CLOSED in production (measured as anon). ⚠ announcer also owns this file — §1 only, tell me before editing that section. |

-- 0146 — the 동반 (self-run) walk finally gets a RECORD. The writer `participant_activities`
-- never had.
--
...
--
--  M1  0146 deleted outright — THE HOLE, UNFIXED. 12 of 13 pins red, and C0 stayed green with
...
begin
  ...
  --   owner's record name their COMPANION dog and not the one they handed to a runner (suite 178 C6).
...
comment on function session_record_companion_run is
  '0146: the 동반 (self-run) walk''s record writer. Sean 2026-08-26 「the self runs are still part
...
begin
  ...
  if v_oid is null then raise exception '0146 VERIFY ①: function absent'; end if;
  if not v_sec then raise exception '0146 VERIFY ①: not SECURITY DEFINER'; end if;
  if v_kind <> 'f' then raise exception '0146 VERIFY ①: prokind=%', v_kind; end if;
  ...
  if not (v_cfg @> array['search_path=public, pg_temp']) then
    raise exception '0146 VERIFY ①: proconfig=% (expected in-body search_path=public, pg_temp)',
  ...
  if v_pub then raise exception '0146 VERIFY ②: PUBLIC can execute'; end if;
  if v_anon then raise exception '0146 VERIFY ②: anon can execute'; end if;
  if not v_auth then raise exception '0146 VERIFY ②: authenticated CANNOT execute — the grant did not land'; end if;
  if not v_svc then raise exception '0146 VERIFY ②: service_role CANNOT execute'; end if;
  ...
  if v_accepted then raise exception '0146 VERIFY ③a: a null-uid caller was ACCEPTED'; end if;
  if v_err <> 'not_signed_in' then
    raise exception '0146 VERIFY ③a: expected not_signed_in, got %', v_err;
  end if;
  ...
  if v_accepted then raise exception '0146 VERIFY ③b: a non-participant was ACCEPTED'; end if;
  if v_err <> 'not_joined' then
    raise exception '0146 VERIFY ③b: expected not_joined, got %', v_err;
  end if;
  perform set_config('request.jwt.claim.sub', '', true);
  ...
  if v_after <> v_before then
    raise exception '0146 VERIFY ③c: refusals wrote % row(s)', v_after - v_before;
  end if;
end $$;

...
| N3 | Every screen that shows the dock should show a selected tab (`components/tab-bars`). | **BUILT 2026-09-22 (`a3f631b`)** — the three dock screens with no tab now highlight their PARENT (`app/src/lib/tab-parent.ts`, 45 pins incl. a drift gate against `bottomnav.tsx`): /alerts and /safety → 마이 for both roles, /cards → 마이 for an owner and 홈 for a runner (whose 마이 row Sean deleted on 2026-08-11). `tabNeighbors` deliberately keeps its exact-path match, so a swipe on these screens still goes nowhere. Was: `active` false for all five, no indicator anywhere. | MEDIUM |
...
| N8 | A sheet needs a grabber when resizable, swipe-to-dismiss, Cancel on the leading edge and Done on the trailing edge (`components/sheets`). | **PARTIAL — owner flow BUILT 2026-09-22 (`3738388`).** New `app/src/components/paper-sheet.tsx:48` is a second real `presentationStyle="pageSheet"` beside `toss-sheet-impl.tsx:70`, and the two sheets in the owner booking flow now use it: the time-slot picker (`owner/request.tsx:1382`, title 「언제 달릴까요?」 moved body → header) and the booking manager (`owner/schedule.tsx:655`, title names the mode — 예약 상세 / 일정 취소). Each gains swipe-down dismiss and a leading 닫기; each LOSES tap-outside-to-dismiss, because iOS does not dismiss a pageSheet on a backdrop tap — both exits land on the same `onClose` the old backdrop and `onRequestClose` already called, so no handler, state or copy changed. **No grabber and no trailing 완료 anywhere, deliberately:** neither sheet resizes (HIG reserves the grabber for that, and the retired 44×5 bars were promising a drag these sheets could not perform), and neither has a commit action to put on the trailing edge — the picker commits on the chip tap and the booking sheet's commit is a destructive button carrying the server's fee quote in its own label. `PaperSheet` therefore ships with no `onDone` prop rather than an arm with no caller. Two Modals in the same files were read and deliberately left: `owner/request.tsx:1435` is a full-screen card gate, not a sheet (a swipe-dismiss there would add an exit that skips `payOnce()`), and `owner/request.tsx:1445` is an alert-like blocking countdown with no `onRequestClose` by design. Measured at that sha in this worktree: **18 `<Modal` sites remain, 2 of them `pageSheet` components covering 3 sheets** — every other screen family (club, runner, chat, settings, community) is still hand-rolled and is the rest of this row. ⚠ Device-visual UNVERIFIED — no simulator or hardware pass. | HIGH |
...
| A3 | Add a hint where the label alone doesn't say what will happen (`foundations/accessibility`). | **BUILT `6305740` (2026-09-22)** — hints + `accessibilityActions` on the three gesture-only controls: `SealSlide` (club 위탁 consent — it had no role, label or hint at all, so its drag AND its 700ms long-press were both unreachable), club chat delete/report, runner photo delete. Each action routes to the same handler through a system confirm. ⚠ The old count here (**1 hint in the whole tree**) was the BEFORE state; the remaining gap is A1/A2's, not this row's. | MEDIUM |
...
| A6 | Announce content that changes without a tap, or it is invisible to VoiceOver (`foundations/accessibility`, `patterns/feedback`). | **BUILT `6305740` (2026-09-22)** — `src/lib/a11y-announce.ts` (de-duped, 1.5s window, last offer wins; 45 pins, mutation-verified) wired to `owner/live`, `runner/run` (+ km milestones, + the end sheet's second step), `owner/radar`, `chat` (link line, and that a message arrived — never the body), `owner/home` and `runner/home`; `accessibilityLiveRegion="polite"` on those strips for Android. ⚠ The count this cell used to carry (**0 / 0 / 0**) is now FALSE — kept here only as the BEFORE state, because a later session grepping this row would otherwise re-do the slice. **Source only: no VoiceOver device pass yet.** | HIGH |
| A7 | Honor Reduce Motion: swap springs and slides for a short cross-fade and keep the static cue (`foundations/accessibility` › Cognitive, `foundations/motion`; DESIGN.md §7c 「Reduced motion is not "no motion"」). | **CEREMONY SURFACES BUILT `fd76490` (2026-09-22)** — the mode→behaviour mapping now lives in ONE pure module, `app/src/lib/motion-policy.ts` (`{reduceMotion, kind:'feedback'\|'decorative'}` → `{animate, travel, durationMs, staggerMs, useNativeDriver}`), pinned by `app/test/motion-policy.test.cjs` (**25 pins**, control-clean and mutation-verified nine ways — every one of `feedback→animate:false`, `travel` surviving reduce, `Math.min→Math.max`, `REDUCED_MS` past the 200 ms cap, the driver moving with the setting, the stagger surviving, decorative keeping a timeline, and `reduceMotion`/`kind` being ignored outright reddens at least one arm). **Sites, `file:line` → decision:** `app/app/owner/report.tsx:1307` patch spring → **feedback** (spring→180 ms cross-fade; `:1341` scale 0.4→1 and rotate −18°→−4° pinned at the landing values) · `:1313` stamp slam → **feedback** (`:1369` scale-from-2.2 and rotate-in pinned at `st.angle`; `:1319` the 80 ms stagger collapses to 0 so the set lands together) · `:1322` copy/CTA → **feedback** (`:1389` the 8 px lift drops, fade only) · `:1324` kicker → **feedback** · `:1325` the three phases run in parallel instead of in sequence under reduce, so the whole ⑫ 수확 ceremony resolves inside one ≤200 ms cross-fade · `app/app/owner/report.tsx:1557/1574` GoalBar → **feedback** (the 700 ms width fill becomes width-at-`pct` + an opacity cross-fade; `layoutProp` keeps it off the native driver in **both** modes, so the node never sees two drivers). ⚠ **`app/app/runner/return-seal.tsx`, `app/app/runner/done.tsx` and `app/src/components/home-hero.tsx` are category (c) — already motion-free**: measured 0 `Animated.`/`LayoutAnimation`/Reanimated sites in each, and R6's 「once per entity」 pop is a rendered line plus a haptic (`return-seal.tsx:115-120,152`), not an animation. They are deliberately NOT wired — a `useReducedMotion()` on a screen with no motion is a dead hook that reads as coverage. home-hero's only motion is `DrawButton`, already covered. 🔴 **`reducedMotion.ts` gained `useReducedMotionState()` (`{reduce, settled}`) and the ceremony screens use it instead of the boolean**, because `isReduceMotionEnabled()` is an async native round-trip that **cannot** resolve before a mount-effect fires: the haul overlay mounts only after its fetches land and starts its animation in the same commit, so the boolean form would have run the full spring-and-slam ceremony **every time** while every gate stayed green. Replaying it to correct that is forbidden (celebrations play once per entity), so the effect WAITS for `settled` and a `started` ref makes a mid-flight toggle a no-op. The once-per-entity Sets are untouched and still consumed at fetch time, so a reduced cross-fade burns the entry exactly as the full ceremony does. **Remaining:** **178 raw / 173 executable `Animated.` sites across 20 files** tree-wide; 4 files now consume the hook (`course/[id].tsx`, `owner/home.tsx`, `owner/report.tsx`, `draw-button.tsx`). The next lanes are the run/live surfaces and the club world. **Source only — no device pass with Reduce Motion ON yet.** | HIGH |
...
| id | rule (HIG page) | how to check it here | sev |
|---|---|---|---|
| S1 | Get permission before sending notifications, and ask in a context that explains the benefit (`patterns/managing-notifications`). | **BUILT 2026-09-22 (`a3f631b`)** — `app/src/components/notification-primer.tsx` now names the three moments per role (owner 러너 배정·픽업·귀가 · runner 새 요청·예약 확정·인계 시각) and fires the system alert on 계속, once per install; `registerPushToken()` no longer prompts at all, so the only path to the alert is the primer's button. Decision pinned pure in `notification-primer-gate.ts` (16 pins). Was: `requestPermissionsAsync` on both homes' mount effects, which for a signed-in user is launch. | HIGH |
...
| 3 | **No AutoFill on any form field** `[inferred]` | **BUILT 2026-09-22 (`a3f631b`)** — sweep of the remaining personal-data fields took `textContentType` and `autoComplete` from **11 to 16** each across the same 51 `<TextInput>` (comment-stripped; raw agrees): 동네/활동 지역/홈 베이스 → `sublocality` (a Korean 동 is not a city), @handle → `nickname` (never `username` — it is not a credential), 상세 → `postal-address-extended`, and 카카오톡 ID → `none`/`off`, a deliberate SUPPRESSION so iOS stops offering the password manager for a third party's handle. Free-text notes, a dog's name and an address label are untouched on purpose. No phone/email field exists (`runner/apply.tsx:716`). Was: 0/0 at the 2026-09-17 audit. Rows E1–E3. | HIGH |
...
| 8 | ~~**VoiceOver has no way to hear a live run change**~~ — **CLOSED `6305740` (2026-09-22)**, source only | The counts that opened this row (0 `accessibilityLiveRegion` · 0 `announceForAccessibility` · 1 `accessibilityHint`) were the BEFORE state and no longer hold — see rows A3/A6 for what landed. What is still open is the **device pass**: nothing here has been heard with VoiceOver on. | HIGH |
...
| 10 | **Most modals are hand-rolled, and iPad/launch config is unset** `[inferred]` | ⚠ **The opening count (18 of 19) is the 2026-09-17 state and no longer holds — see row N8.** `3738388` (2026-09-22) added `app/src/components/paper-sheet.tsx:48` as a second real `presentationStyle="pageSheet"` and converted the owner flow's two sheets to it; measured at that sha, 18 `<Modal` sites remain with 2 pageSheet components covering 3 sheets. Every other screen family is still `transparent animationType="slide"` — no detent, no grabber, no swipe-dismiss. Separately: `app.json` sets `ios.supportsTablet: true` **with** `orientation: "portrait"` and has **no** `splash` block and **no** `ios.icon` (so no dark/tinted icon variants). Rows N8, L5, O1, W9. | MEDIUM |

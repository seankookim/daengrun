...
| N8 | A sheet needs a grabber when resizable, swipe-to-dismiss, Cancel on the leading edge and Done on the trailing edge (`components/sheets`). | **PARTIAL — owner flow (`3738388`) and club/shot sheets BUILT 2026-09-22.** The owner time-slot picker and booking manager already use `PaperSheet`; this slice converts seven additional inline sheets to `presentationStyle="pageSheet"`: club session creation; session seat confirmation, add-dog picker, RSVP picker, host conversation and reason input; shot photo picker. Each has a leading 취소/닫기 and `onRequestClose`; native swipe dismissal is explicitly enabled, except while the add-dog request is busy (the existing close guard). Trailing 완료 invokes the existing commit only for session creation, seat confirmation and photo selection; row pickers, chat and the reason sheet keep their specifically labelled actions instead of inventing a second commit. Decorative grabbers and tap-outside backdrops are removed; these sheets do not resize. Native safe-area views replace capped bottom panels. The full-screen shot studio/camera flow is unchanged. `community.tsx`, `settings.tsx` and `my.tsx` have no inline bottom-sheet Modal to convert; shared component sheets and other screen families remain outside this slice. Historical global modal counts from the owner-flow audit are not reasserted. **Device-visual UNVERIFIED:** simulator/hardware sheet sizing, keyboard and swipe checks remain. | HIGH |

...
6. **Re-measure every baseline on `origin/redesign-v4` HEAD.** Save each log. Read its exit code and the whole output, never through `tail`.
   ...
   - Also run `npm run lint` from `app/` and count its errors. GitHub's `gates` workflow (`client-gates` job) fails when there are more than 6, and trunk `0ee30fa` was already red there: **11 errors** (measured 2026-09-25, CI run 36140102449, then reproduced locally). The local commit gate does not run lint, so a green local chain says nothing about this check.
...
| # | Item | Evidence |
|---|---|---|
| F0 | **Trunk CI is red: lint.** `client-gates` counts 11 `react-hooks/exhaustive-deps` errors against a baseline of 6. Measured at `0ee30fa`:<br>• `owner/course-map.tsx:170` (`pickup`)<br>• `owner/fitness.tsx:113` (`load`)<br>• `owner/matching.tsx:232` (`pickMode`) and `:293` (`rebook`)<br>• `owner/request.tsx:196` (`pickup`) and `:206` (`autoPick`)<br>• `runner/calendar.tsx:136` (`load`)<br>• `runner/done.tsx:220` and `:241` (`bookingId`)<br>• `runner/earnings.tsx:181` (`load`)<br>• `shop.tsx:79` (`load`)<br>**Judge each site; never add a dependency mechanically.** A missing dependency can be a stale-closure bug, while adding an unstable one can loop an effect or re-run a fetch. Stabilise `load` with `useCallback`, or read the value from a ref.<br>• `matching.tsx` is held by 0211, so fix only its lint lines, in their own commit.<br>• Each site's file belongs to the open slice that owns it (P8/P10). Otherwise it goes to one small `fix/lint-exhaustive-deps` slice that lands **first**, because every PR's CI is red until it does. | CI run 36140102449; local `npm run lint` |

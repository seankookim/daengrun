...
22. **Six build items the spec gap finder (2026-09-21 01:47) found GATED on you** — each is one word: (a) the
    ...
    build's main nav while the spec says demo zones never sit in main nav — ⓐ drop it (4-tab bar) · ⓑ keep; ⚠ **2026-09-26 — /shop folded in here (no new letter), with a second fact:** the tab (`app/src/components/bottomnav.tsx:42`) renders each product's `collab` line (`app/app/shop.tsx:247`), and the data names third-party brands as partners — 「도그스하이 × 바잇미」 (`app/src/store.ts:227`), also 「도그스하이 × 페스룸」 (:228) and 「도그스하이 × 페티즌」 (:232) — under a 「· 오픈 준비 중」 header (`shop.tsx:237`) and 「예정」 prices (`shop.tsx:254`). The 「준비 중」 framing covers the store opening, not the partnership: if no such agreement exists, the line states a partnership the company does not have. So (b)'s options become: ⓐ drop the tab (4-tab bar; the claim leaves with it) · ⓑ keep the tab, remove the × brand names until an agreement exists · ⓒ keep as is (an agreement exists).
...
0-strand. **🟡 2026-09-26 04:4x KST — CUSTODY STRAND: ONE QUESTION (merges 0-custody ⓐ/ⓑ and 0-sweep #16, which contradicted each other).** The two letters asked the same thing with opposite answers: 0-custody (19:30) suggested **60 / 240 minutes and no ops force-end** (resolve by phone first); 0-sweep #16 (17:08, backend-logic-1) suggested **30 / 60 minutes and an ops force-end** with a reason token. Facts, read on trunk `0ee30fa`: the two values are `ops_flags.custody_start_strand_minutes` / `custody_end_strand_minutes`, which ship NULL = off (`0224:39-40,175`), and setting them is one `update ops_flags` (`0224:162`); both operator doors that could end a run refuse one the runner never stopped — `force_return_tx` (latest body `0218:265`) and `ops_resolve_return_tx` (latest body `0224:1686`) raise `run_not_ended` — so a force-end is a NEW money-path arm (a migration, and ending a run freezes money), while thresholds are a flag write. **Options:** ⓐ 30 min after both handoff stamps / 60 min past km×8+25, AND build an ops force-end (new journaled `force_return_tx` arm, reason token, settles at recorded distance) · ⓑ 60 / 240, NO force-end yet: bell + accept gate only; the runner or owner acts, ops resolves by phone; revisit force-end once a real strand has happened · ⓒ your own two numbers (name them), and say force-end yes or no. **Recommendation: ⓑ.** Thresholds can be tightened later by one flag write; a force-end door cannot be un-shipped cheaply and needs the full adversarial cycle on a money path, for a state no pilot run has produced yet. #16's case for ⓐ is real and stays on the record: a dog whose runner never taps is the one state nobody else can close. 0-custody's ⓒ (does a LIVE run block new accepts) and ⓓ (who is on the rosters) are NOT merged — they stay open under 0-custody.

0-custody. ⚠ **ⓐ and ⓑ MERGED INTO 0-strand (2026-09-26); ⓒ and ⓓ still stand here.** **🟡 19:30 — 0224 CUSTODY STRAND THRESHOLDS (four questions, none urgent — the sweep ships OFF).** 0224 (`f131f28`) watches two custody shapes nobody resolved before: a picked-up dog whose run never STARTED, and a run that never ENDED; it bells the `return_strand` roster, tells both parties, and — only while a threshold is set — holds the stranded runner's new accepts. Also a `payout_due` bell for sealed-but-unsettled runs older than 6 h. ⓐ **Thresholds:** `ops_flags.custody_start_strand_minutes` / `ops_flags.custody_end_strand_minutes` (names verified in 0224 at 19:3x) — my suggestion 60 / 240, set only after the client half (fix/custody-strand-client, building now) lands; ⓑ **may ops force-start / force-end a custody?** Today the console lists are read-only (recommend: not yet — resolve by phone first); ⓒ **should a LIVE run also block new accepts?** (0092 §3 rejected it; your 2026-08-13 ruling can be read either way — recommend no); ⓓ **rosters:** who is on `return_strand` and `payout_due` in production (your `ops_recipients` rows from 09-22 cover `payout_due` and `return_strand`, measured then).

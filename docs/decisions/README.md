...
| ⑪ | [incident-verification.md](incident-verification.md) | Who verifies an `incident` | ✅ **RULED + BUILT + DEPLOYED (0094/130).** *"incident verified by both runner and owner."* Opening is one-sided (a dog may be bleeding); ESTABLISHING is two-sided; and the phone door opens on the OPEN, not the verification — gating it on `verified_at` deadlocks the emergency, and mutation P3 reddens both 130 V3 and 0088's own G7. Also closed `0002:154`, an INSERT policy that let any user open an incident on a stranger's booking and trip that booking's phone exchange. | **server only — no client surface**, and `appstore-privacy-answers.md:27` must move before it ships to users |
| ⑫ | [marketplace-incident-exit.md](marketplace-incident-exit.md) | ✅ **RULED + BUILT + DEPLOYED (0092/128).** *"pay the runner but dont let them make new runs until the dog is confirmed by both sides."* The runner is paid; the counterweight is a work gate, not a payment condition — which dissolves the question codex refused rather than answering it. ⚠ The "both sides" is the DOG'S RETURN (0083 stamps), NOT ⑪ — this set claimed otherwise and was wrong. ⑪ is independent and still unbuilt. |

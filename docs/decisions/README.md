...
| 5 | Per-runner G1 telemetry + owner-visible condition_note | Auto (both voices) | P1 | STILL REQUIRED — not in 0080; next money slice |
...
| 13 | Reconciliation arms per marker class + heartbeat | Auto (both voices) | P1 | LARGELY SATISFIED (0080 reconciliation 4-arm + 15-min verification arm); >0-rows loud-log heartbeat cron = still open, next slice |

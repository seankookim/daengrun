...
- [x] 빌드 10 (4a59201): 러닝 사진(런 화면 📷 → 전 마리 업로드+보호자 알림 — 영수증 인화 원천) + 챗 사진(3곳 입력바·사진 말풍선)
- [ ] [ready] 폴라로이드 승급(러너 챗 사진→보호자 라이브) · 케이스 타임라인에 커스터디 이벤트 병합 · 단체샷 의식(세션 종료 시 호스트 프롬프트→강아지의 책 팬아웃)

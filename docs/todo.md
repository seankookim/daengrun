...
- [ ] **Sean: `npx supabase db push` (0025 패치 보너스 + 0026 반복 예약 + 0027 집계 RPC + 0028 정산 이넘 캐스트 핫픽스)** — 함수 재배포는 불필요 (RPC/크론만). ⚠ 0027 push 전까지 앱의 포인트 잔액·정산 예정이 RPC 404로 에러 — push 먼저, 앱 확인 나중. ⚠ **0028 push 전까지 모든 정산이 이넘 타입 에러로 실패** (2026-07-29 실기기 발견 — 트랜잭션 롤백이라 부분 반영은 없음)

...
| Cancel-fee mirror | **SHIPPED to `claude/cancel-fee-mirror` @ `c5bc01d`** — cut from stage2 (quote_cancel_fee in-tree) with trunk merged in; strict-parse quote, failed quote BLOCKS the commit button, %-constants retired, cancelFeeRateFor deleted. **This branch IS the 0117 deploy unit: merging IT to trunk at deploy time ships migration + client together.** Never land the client half early (DOG_SELECT-400 class) and never land 0117 without it. Needs one `git merge origin/redesign-v4` before its landing (the 0118/0119 landings postdate its cut) — ui6/successor runs it. Branch existence + stage2 ancestry [verified-now]; the client behavior claims are ui6's gates (tsc 0 · rpc ✅ · lint baseline 6 · 0 FAIL) [reported] |

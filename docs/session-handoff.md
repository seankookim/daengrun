...
| Trunk `redesign-v4` | `1e7c6f6`+ — carries ui6's 10 UI-pick commits + C④ + the 도그스하이 orderName edit (deno 223/0) | [verified-now] |
| **0118 club fee** | `claude/club-fee-slice` @ `0443184`. **COMPLETE + Sean's land word GIVEN.** 747/0, four adversarial rounds, final verdict LAND, ruling B implemented+measured (runnerless cancel = platform half only, 3 pins moved with it, revert-mutation reds P1/P14/ARM5). **NOT YET MERGED to trunk — the next session's FIRST ACT** (see §6 for the one open anomaly) | [verified-now] |
| **0119 맹견 gate** | `claude/wf-maenggyeon` @ `b28a787` — ui6's trunk-merge DONE (dog.tsx/requests.tsx hand-resolved, gates green on tip: tsc 0 · rpc ✅ · route-native ✅ · 8 suites 0 FAIL). Landing in progress this session | [reported] gates are ui6's numbers; ancestor check re-verified at merge time |
| **0117 late-booking** | branch fat and healthy (796/0 · deno 232/0, R1-R17 + 4B + R10-13 all closed) but **NOT landing**: §4.2's two statement-money arms are UNRULED, and the client fee-mirror ships ATOMIC WITH its deploy | [verified-now] |
...
| Cancel-fee mirror | **ui6's, IN FLIGHT** (Sean: "ship the cancel fee mirror thing"). ⚠ Ships WITH 0117's deploy, never before — quote_cancel_fee lives on the 0117 branch; a client reading an absent RPC is the DOG_SELECT-400 class. **Do not "helpfully" land it on trunk early** | [reported, their words] |
